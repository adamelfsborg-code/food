...
import { AddCategoryResponseSchema, CategoryCreateDtoSchema, CategoryDeleteDtoSchema, CategoryEditDtoSchema, CategoryFilterDtoSchema, CategoryTableSchema, DeleteCategoryResponseSchema, EditCategoryResponseSchema, GetCategoryResponseSchema } from "@/lib/schema/category";
...
export const ListCategoriesAPI = async (props: unknown) => {
  ...
  const response = await fetch(`${parsedEnv.API_CULINARY_ADDR}/categories/list`, {
    method: 'GET',
    headers: {
  ...
  const responseSchema = GetCategoryResponseSchema.safeParse(result)
...
export const GetCategoryAPI = async (props: unknown) => {
  ...
  if (!categorySchema.success) return {
    success: false,
    error: categorySchema.error.message
  }


  const response = await fetch(`${parsedEnv.API_CULINARY_ADDR}/categories/${categorySchema.data.id}`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${token}`
...
export const AddCategoryAPI = async (props: unknown) => {
  ...
  const result = await response.json();
  const responseSchema = AddCategoryResponseSchema.safeParse(result)
...
export const EditCategoryAPI = async (props: unknown) => {
  ...
  const result = await response.json();
  const responseSchema = EditCategoryResponseSchema.safeParse(result)
...
export const DeleteCategoryAPI = async (props: unknown) => {
  ...
  const result = await response.json();
  const responseSchema = DeleteCategoryResponseSchema.safeParse(result)
